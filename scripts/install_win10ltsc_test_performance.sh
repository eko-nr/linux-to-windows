...
# Optimized for nested virtualization & emulator performance
# ============================================================
...
if (( VCPU_COUNT < 2 )); then warn "Raising vCPU to 2 for emulator baseline"; VCPU_COUNT=2; fi  # [OPTIMIZATION]
...
# [OPTIMIZATION] Compute sensible topology: prefer 2 threads per core if possible
...
sudo virt-install \
  ...
  --cdrom "${ISO_LINK}" \
  --disk path="/var/lib/libvirt/images/${VM_NAME}.img",size="${DISK_SIZE}",bus=scsi,discard=unmap,detect_zeroes=unmap,cache=writeback \
...
# --- Enable Nested Virtualization (host module param) ---
...
# [OPTIMIZATION] Ensure hypervisor hints that can mislead Windows/Emulator are minimized
# Remove Hyper-V enlightenments to avoid WHPX fallback inside Windows guest
# (We intentionally do NOT set --features hyperv_*)
sudo virt-xml "${VM_NAME}" --edit --features acpi=on,apic=on 2>/dev/null || true

# [OPTIMIZATION] Add virtio RNG & balloon explicitly (better responsiveness)
...
# [OPTIMIZATION] Try to disable CPUID hypervisor bit (best-effort; harmless if not supported)
# Not all virt-xml versions support this flag; ignore if it fails
sudo virt-xml "${VM_NAME}" --edit --cpu host-passthrough,check=none 2>/dev/null || true
...
ok "CPU pinning applied (best effort)"
